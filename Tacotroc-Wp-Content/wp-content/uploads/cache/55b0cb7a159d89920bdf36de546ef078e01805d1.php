<article <?php post_class() ?>>
  <main>
    ...
    <?php
        ...
        $query = "SELECT p.ID, p.post_title, (SELECT SUBSTRING(SUBSTRING_INDEX(SUBSTRING_INDEX(meta_value,';',2),':',-1), 2, LENGTH(SUBSTRING_INDEX(SUBSTRING_INDEX(meta_value,';',2),':',-1)) - 2) FROM wp_postmeta WHERE post_id=p.ID and meta_key='modele') as modele, (SELECT meta_value FROM wp_postmeta WHERE post_id=p.ID and meta_key='type') as type FROM wp_posts as p WHERE p.post_status='publish' AND p.post_type='".$type."' HAVING modele IN (SELECT ID FROM wp_posts WHERE wp_posts.`post_type`='modeles' AND post_name like '". get_the_title() ."%' AND post_status='publish') LIMIT 20 OFFSET ".$offset." ";
    ...
    <section id="Offers">
        <div id="recentOffers">
            <p id="menuOffer">
              <a href="?type=offre"><span id="activeOffer">Offres</span></a>
              <a href="?type=demande"><span>Demandes</span></a>
              <a href="?type=echange"><span>Échanges</span></a>
            </p>
        ...
        <div id="clubs">
            <p id="clubTitle"><img src="<?= App\asset_path('images/hr.png'); ?>" alt="ligne horizontale" id="hr" />Clubs</p>
