<?php
  ...
  $query = 'SELECT ID FROM wp_posts WHERE post_status = "publish" AND post_type = "post" AND ID not in ('.$sticky.') LIMIT 20 OFFSET '.$offset;
...
<p class="path" style="cursor:pointer">
  <span onclick="window.location.href='index.phtml'">Accueil</span>
    ...
    <article id="last">
        ...
        <?php foreach($singles as $single): ?>
          <a href="<?= get_permalink($single->ID) ?>">
            <div class="articles">
                <div class="articlePicture" style="background-image: url(<?= get_the_post_thumbnail_url($single->ID) ?>)"></div>
...
<?php
  if(count($singles) > 0):
    ...
    paginationBlog($nbResults, $page, 20, "/blog/page/");
  endif;
?>
